...
local function CreateEntry(name, reason)
   ...
   if listMode == "blacklist" then
    entry.reason:SetTextColor(1, 0, 0) -- red
elseif listMode == "whitelist" then
    entry.reason:SetTextColor(0, 1, 0) -- green
elseif listMode == "graylist" then
    entry.reason:SetTextColor(0.7, 0.7, 0.7) -- gray
end
...
local function ShowTextWindow(title, initialText, onAccept)
    ...
    scrollFrame:SetSize(300, 130) -- fixed size scrollable area
...
local function AddNuggetIconToChat(self, event, msg, author, ...)
    ...
    elseif IsGraylisted(author) then
