...
local function UpdateModeButton()
    if listMode == "blacklist" then
        ...
        modeToggleButton:GetFontString():SetTextColor(1, 0, 0) -- red
    else
        modeToggleButton:SetText("Whitelist")
        modeToggleButton:GetFontString():SetTextColor(0, 1, 0) -- green
    end
end

modeToggleButton:SetScript("OnClick", function()
    listMode = (listMode == "blacklist") and "whitelist" or "blacklist"
    UpdateModeButton()
    LoadList()
end)
...
local toggleDeleteButton = CreateFrame("Button", nil, frame, "UIPanelButtonTemplate")
toggleDeleteButton:SetSize(130, 25)
toggleDeleteButton:SetText("Auto Delete: On")
toggleDeleteButton:SetScript("OnClick", function()
    autoDeleteEnabled = not autoDeleteEnabled
    toggleDeleteButton:SetText("Auto Delete: " .. (autoDeleteEnabled and "On" or "Off"))
end)

local totalWidth = 80 + 80 + 80 + (10 * 2)
local startX = (frame:GetWidth() - totalWidth) / 2

addButton:SetPoint("BOTTOMLEFT", frame, "BOTTOMLEFT", startX, 27)
importButton:SetPoint("LEFT", addButton, "RIGHT", 10, 0)
exportButton:SetPoint("LEFT", importButton, "RIGHT", 10, 0)
toggleDeleteButton:ClearAllPoints()
toggleDeleteButton:SetPoint("BOTTOM", frame, "BOTTOM", 0, 2)
...
local function AddNuggetIconToChat(self, event, msg, author, ...)
    if IsBlacklisted(author) then
        msg = nuggetIcon .. msg
    elseif IsWhitelisted(author) then
